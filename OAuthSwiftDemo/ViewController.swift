...
extension ViewController {
   ...
   func doAuthService(service: String) {
      ...
      case "TrainingPeaks":
         doOAuthTrainingPeaks(parameters)
      case "Tumblr":
   ...
   func doOAuth500px(serviceParameters: [String:String]){
      ...
      oauthswift.authorizeWithCallbackURL( NSURL(string: "oauth-swift://oauth-callback/500px")!, success: {
   ...
   func doOAuthTrainingPeaks(serviceParameters: [String:String]){
      let oauthswift = OAuth2Swift(
         consumerKey:    serviceParameters["consumerKey"]!,
         consumerSecret: serviceParameters["consumerSecret"]!,
         authorizeUrl:   "https://oauth.sandbox.trainingpeaks.com/oauth/authorize",
         accessTokenUrl: "https://oauth.sandbox.trainingpeaks.com/oauth/token",
         responseType:   "code"
      )
      let state: String = generateStateWithLength(20) as String
      oauthswift.authorizeWithCallbackURL( NSURL(string: "oauth-swift://oauth-callback/trainingpeaks")!, scope: "file:write", state: state, success: {
         credential, response, parameters in
         self.showTokenAlert(serviceParameters["name"], credential: credential)
         }, failure: { error in
